...
class Sitio extends Controller
{
    ...
    public function changeExchangeRate(Request $request) {
        $data = $request->all();
        if ($data['type_change'] == 1) {
            $dataExchange = json_decode( file_get_contents($this->API_HOST.'/api/v1/finanzas/type-change/get'), true );
            session(['exchangeValue' => $dataExchange['data'][0]['change']]);
        } else if ($data['type_change'] == 2) {
            $dataExchange = json_decode( file_get_contents($this->API_HOST.'/api/v1/finanzas/type-change/get'), true );
            session(['exchangeValue' => $dataExchange['data'][1]['change']]);
        } else {
            session(['exchangeValue' => 1]);
        }
        session(['exchangeRate' => $data['type_change']]);
    ...
    {
        $dataCategoriesTours = json_decode( file_get_contents($this->API_HOST.'/api/v1/categorie-tours/get'), true );
        ...
        return view('home',compact('categories','title','meta_description','keywords','hotels','zones', 'dataCategoriesTours'));
    }

    public function getPaquetes() {
        $dataPaquetes = json_decode( file_get_contents($this->API_HOST.'/api/v1/package-tours'), true );
        $dataPaquetes = $dataPaquetes['data'];
        return view('paquetes', compact('dataPaquetes'));
    }
    ...
    public function cambio()
    {
        ...
        $categories = Categorie::all()->where('active', '=',true);
        $keywords = "tours en cancun, riviera maya, tours economicos,chichen itza, ruinas mayas, actividades en cancun";
        return view('cambio',compact('categories','title','meta_description','keywords','hotels','zones'));
    }
    ...
    public function movil()
    {
        ...
        $categories = Categorie::all()->where('active', '=',true);
        $keywords = "tours en cancun, riviera maya, tours economicos,chichen itza, ruinas mayas, actividades en cancun";
        return view('movil',compact('categories','title','meta_description','keywords','hotels','zones'));
    }
     ...
     public function departamentos()
    {
     ...
     public function EnDepartamento()
    {
        ...
        // $categories = Categorie::all()->where('active', '=',true);
        // $keywords = "tours en cancun, riviera maya, tours economicos,chichen itza, ruinas mayas, actividades en cancun";
         return view('en.departamentos');
    }

     public function HotelesAviones()
    {
      
         return view('hotel_avion');
    }
...
//
        ...
        //echo $datos['servicio'];
        //);
         return view('transporte',compact('datos','vans','tipo_servicio','price','hotels'));
    }
    ...
    public function transportacionGet()
    {
        ...
        $zones = Zone::all();
        return view('transporte',compact('vans','price','hotels','zones'));
    }

     public function transportacionN()
    {
        $hotels = Hotel::all();
        $price='';
        $vans = Van::all()->where('type','=',false);
        return view('n_transporte',compact('vans','price','hotels'));
    }
    ...
    public function cars()
    {
         ...
         $vans = Van::all()->where('type','=',1)->where('idioma','=','');
        
        return view('cars',compact('vans','price','hotels'));
    }
    ...
    public function yates()
    {
         $hotels = Hotel::all();
         $price='';
         $vans = Van::all()->where('type','=',2)->where('active','=',0)->where('idioma','=','');
        return view('yates',compact('vans','price','hotels'));
    }
    ...
    public function nosotros_email(Request $request)
    {
        ...
        Mail::to('reservas@cancundoit.com')->send(new Email_nosotros($request->all()));
        // Mail::to('pablodelhip@gmail.com')->send(new Email('holaaa'));
        return \Redirect::route('hotel_avion')
                 ->with('message', 'El email fue enviado de forma correcta');
        // return view('nosotros');
    }
    ...
    {
        $Categorias = Categorie::all()->where('url', '=',$url)->where('idioma', '=','');
    ...
    {
        return view('ofertas.oferta-ingles');
    }
    ...
    public function infoTour($tour = '')
    {
        ...
        //dd($this->API_HOST.'/api/v1/tours/get/info/tour/name/'.$tour);
        // echo "hola";
        $dataTours = json_decode( file_get_contents($this->API_HOST.'/api/v1/tours/get/info/tour/name/'.$tour), true );
        // dd($dataTours);
        $tour = $dataTours["data"]["tour"][0];
        $seo_tour = $dataTours["data"]["seo_tour"][0];
        $general_information = $dataTours["data"]["general_information"][0];
        $operation_tour = $dataTours["data"]["operation_tour"][0];
        $image_tour = $dataTours["data"]["image_tour"];

        // dd($operation_tour);
        $precio_final_adulto = $operation_tour["adult_price"] / $tipoCambio;
        $precio_final_nino = $operation_tour["child_price"] / $tipoCambio;
        if($operation_tour["discount_rate"] != 0) {
            $precio_final_adulto = (($operation_tour["adult_price"] * $operation_tour["discount_rate"]) / 100) / $tipoCambio;
            $precio_final_nino = (($operation_tour["child_price"] * $operation_tour["discount_rate"]) / 100) / $tipoCambio;
        }
        
        
        $precio_final_adulto = number_format($precio_final_adulto, 2, '.', ',');
        $precio_final_nino = number_format($precio_final_nino, 2, '.', ',');
        $precio = number_format($operation_tour["adult_price"] / $tipoCambio, 2, '.', ',');
        $title = $seo_tour["title"]. ' | Caribbean Holidays';
        $meta_description = $seo_tour['meta_description'];
        $keywords = $seo_tour['keywords'];
        $dias_anticipo_reserva = $operation_tour["days_advance_to_reserve"];
        

        $dias_activo_tour="";
        if($operation_tour['mon']==0)
            {
                $dias_activo_tour.="day != 1 && ";
            }
            if($operation_tour['tue']==0)
            {
        ...
        return view('infoTour',compact('tour','seo_tour','general_information','operation_tour','image_tour', 'precio_final_adulto', 'precio_final_nino', 'precio', 'title', 'meta_description', 'keywords', 'dias_anticipo_reserva', 'dias_activo_tour'));
